...
export function DataSyncStatus() {
  ...
  const checkPendingData = () => {
    try {
      ...
      const getStarted = JSON.parse(localStorage.getItem('scaro_getstarted') || '[]');
  ...
  const syncPendingData = async () => {
    ...
    try {
      ...
      for (const submission of pendingGetStarted) {
        try {
          const response = await fetch('/api.php?action=get-started', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
      ...
      localStorage.setItem('scaro_getstarted', JSON.stringify(getStarted));
