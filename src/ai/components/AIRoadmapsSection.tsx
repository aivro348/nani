...
export const AIRoadmapsSection = memo(function AIRoadmapsSection() {
    ...
    <div id="roadmaps" className="py-24 px-4 bg-slate-50 dark:bg-surface scroll-mt-24">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center max-w-3xl mx-auto mb-20">
          ...
          >
            AI Career <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-cyan-500">Roadmaps</span>
          </motion.h2>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="absolute top-0 right-0 w-64 h-64 bg-purple-500/5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/4 pointer-events-none" />
          
          <div className="text-center mb-12 relative z-10">
            <span className="inline-flex items-center gap-2 px-4 py-2 bg-purple-500/10 text-purple-600 rounded-full text-sm font-bold tracking-wide uppercase mb-4 border border-purple-500/20">
          ...
          <div className="relative z-10">
            <div className="hidden md:block absolute top-[28px] left-[12%] right-[12%] h-1.5 bg-slate-100 rounded-full">
              <div className="absolute inset-0 bg-gradient-to-r from-purple-500/20 via-purple-500/50 to-cyan-500/20 rounded-full" />
            </div>
              ...
              {[
                ...
                <div key={i} className="relative flex flex-row md:flex-col items-center md:text-center gap-6 md:gap-5 group">
                  <div className="relative shrink-0">
                    <div className="w-16 h-16 bg-white dark:bg-surface rounded-full border-4 border-slate-100 dark:border-border flex items-center justify-center shadow-lg group-hover:border-purple-500/50 group-hover:scale-110 transition-all duration-300 z-10 relative">
                    ...
                    <div className="absolute -top-1 -right-1 w-6 h-6 bg-slate-800 dark:bg-purple-600 text-white text-xs font-bold flex items-center justify-center rounded-full border-2 border-white shadow-md group-hover:bg-purple-600 transition-colors z-20">
                      {i + 1}
                    </div>
                    {i < 3 && (
                  ...
                  <div className="flex-1 md:mt-2">
                    <div className="inline-block text-[10px] font-black uppercase tracking-widest text-purple-600 mb-2 bg-purple-500/10 px-2.5 py-1 rounded-md">
                      {ph.weeks}
                    </div>
                    <h4 className="text-xl font-black text-heading mb-1.5 group-hover:text-purple-600 transition-colors">
                      {ph.label}
                    </h4>
              ...
              >
                ...
                <div className="p-8 md:p-10">
                  <div className="flex items-start justify-between mb-6">
                    <div className="w-16 h-16 bg-gradient-to-br from-purple-500 to-cyan-500 text-white rounded-2xl flex items-center justify-center shadow-lg">
                      <IconComp className="w-8 h-8" />
                    </div>
                    {roadmap.hotBadge && (
                      <span className="bg-purple-500/10 text-purple-600 text-xs font-black px-3 py-1.5 rounded-full border border-purple-500/20">
                        {roadmap.hotBadge}
                      </span>
                    )}
                  </div>

                  <h3 className="text-2xl font-black text-heading mb-2 group-hover:text-purple-600 transition-colors">
                    {roadmap.title}
                  </h3>
                  ...
                  <div className="flex flex-wrap gap-2 mb-8">
                    {roadmap.skillsGained.map((skill, i) => (
                      <span key={i} className="px-3 py-1 bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300 text-xs font-bold rounded-lg border border-purple-100 dark:border-purple-800/30">
                        {skill}
                      </span>
                    ))}
                  </div>
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 text-purple-600 font-bold text-sm hover:text-purple-700 transition-colors mb-6"
                  >
                      ...
                      {roadmap.weeks.map((week) => {
                        ...
                        return (
                          <div key={week.week} className="flex items-start gap-4 p-4 bg-slate-50 dark:bg-surface rounded-xl border border-slate-100 dark:border-border">
                            <div className="w-10 h-10 bg-purple-500/10 text-purple-600 rounded-lg flex items-center justify-center shrink-0 text-sm font-black">
                              W{week.week}
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1">
                                <h5 className="font-bold text-heading text-sm">{week.title}</h5>
                                <span className="text-[10px] font-bold uppercase text-purple-600 bg-purple-500/10 px-2 py-0.5 rounded hidden sm:inline">{week.phase}</span>
                              </div>
                  ...
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-purple-600 to-cyan-500 hover:from-purple-700 hover:to-cyan-600 text-white font-bold py-4 rounded-xl shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2 active:scale-[0.98]"
                  >
                    View Full Roadmap
