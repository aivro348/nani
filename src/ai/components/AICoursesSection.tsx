...
export const AICoursesSection = memo(function AICoursesSection() {
  const navigate = useNavigate();
  const activeCategories = AI_CATEGORIES.filter(c => c.id !== 'all' && aiCourses.some(course => course.category === c.id));
    ...
    <div id="courses" className="py-24 px-4 scroll-mt-24">
      <div className="max-w-7xl mx-auto space-y-24">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          >
            Master <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-cyan-500">Artificial Intelligence</span>
          </motion.h2>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            ...
            <div key={category.id} className="space-y-12">
              <div className="flex justify-center">
                <motion.div 
                  ...
                  viewport={{ once: true }}
                  className="bg-gradient-to-r from-purple-600 to-cyan-500 text-white px-10 py-3 rounded-xl text-2xl md:text-3xl font-bold shadow-lg text-center"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {categoryCourses.map((course, idx) => (
                  ...
                  >
                    <h3 className="text-[1.35rem] font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-cyan-500 mb-6 tracking-tight leading-tight">
                      {course.title}
                    </h3>
                    ...
                    <button 
                      ...
                      className="bg-gradient-to-r from-purple-600 to-cyan-500 hover:from-purple-700 hover:to-cyan-600 text-white px-7 py-3 rounded-xl font-bold text-sm transition-all w-fit shadow-md hover:shadow-xl hover:-translate-y-0.5 active:scale-95"
                    >
                      View Curriculum
                    </button>
                  </motion.div>
