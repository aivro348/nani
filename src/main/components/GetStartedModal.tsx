...
export function GetStartedModal({ isOpen, onClose }: GetStartedModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}api.php?action=get-started`, {
        method: 'POST',
        headers: {
      ...
      try {
        const submissions = JSON.parse(localStorage.getItem('scaro_getstarted') || '[]');
        submissions.push({
          ...submissionData,
          id: `local_${Date.now()}`,
          submittedAt: new Date().toISOString()
        });
        localStorage.setItem('scaro_getstarted', JSON.stringify(submissions));
      ...
      <DialogContent className="w-[95%] sm:max-w-[500px] bg-card-bg border-card-border text-heading p-4 sm:p-6 theme-transition">
        <DialogHeader>
          <DialogTitle className="text-xl sm:text-2xl bg-gradient-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent flex items-center gap-2">
            ...
            Get Started with Scaro Technologies
          </DialogTitle>
